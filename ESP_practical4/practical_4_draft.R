...
meanList <- function(my_list){
  ...
  for (i in 2:length(my_list)){
    mean_1 <- mean_1[[1]] + my_list[[i]][[1]]
    mean_2 <- mean_2[[1]] + my_list[[i]][[2]]
    mean_3 <- mean_3[[1]] + my_list[[i]][[3]]
  }
  mean_list <- append(mean_list, list(mean_1/length(my_list)))
  mean_list <- append(mean_list, list(mean_2/length(my_list)))
  mean_list <- append(mean_list, list(mean_3/length(my_list)))
  return (mean_list)
}
...
train <- function(nn, inp, k, eta = .01, mb = 10, nstep = 10000) {
  ...
  for (epoch in 1:nstep) {
    ...
    for (i in batch_index){
      nn <- forward(nn, inp[i, ])
      nn <- backward(nn, k[i])
      batch_dW <- append(batch_dW, nn$dW)
      batch_db <- append(batch_db, nn$db)
    }
    mean_dW <- meanList(batch_dW)
    mean_db <- meanList(batch_db)
    for (l in length(nn$W):1) {
      nn$W[[l]] <- mean_dW[[l]] - eta * mean_dW[[l]]
      nn$b[[l]] <- mean_db[[l]] - eta * mean_db[[l]]
    }
    # cat(nn$W[[1]], '\n')
  }
  return(nn)
...
train <- function(nn, iris, k, eta = .01, mb = 10, nstep = 10000) {
  # n <- nrow(inp)
  for (epoch in 1:nstep) {
    batch_dW <- list()
    batch_db <- list()
    batch_index <- sample(train_indices, mb, replace = FALSE)
    for (i in batch_index){
      # print(dim(iris[i, 1:4]))
      # print(iris[i, 1:4])
      # print(k[i])
      nn <- forward(nn, iris[i, 1:4])
      nn <- backward(nn, k[i])
      # print(nn$h[[4]])
      batch_dW <- append(batch_dW, list(nn$dW))
      batch_db <- append(batch_db, list(nn$db))
    }
    mean_dW <- meanList(batch_dW)
    mean_db <- meanList(batch_db)
    for (l in length(nn$W):1) {
...
test_indices <- seq(5, nrow(inp), by = 5)
train_indices <- setdiff(1:nrow(inp), test_indices)
k <- as.numeric(factor(iris$Species, levels = unique(iris$Species)))
test_data <- inp[test_indices, ]
# numeric_category <- as.numeric(factor(iris$Species, levels = unique(iris$Species)))
test_labels <- k[test_indices]
train_data <- inp[train_indices, ]
train_labels <- k[train_indices]

nn <- netup(c(4, 8, 7, 3))
network <- train(nn, iris, k, eta=.01, mb=10, nstep=10000)
...
for (i in 1:length(train_indices)){
  prediction <- forward(network, iris[i, 1:4])$h[[4]]
  # print(train_prediction)
  train_predict_label <- which.max(prediction)
  train_prediction <- append(train_prediction, train_predict_label)
}
train_accuracy <- sum(train_prediction == train_labels) / length(train_prediction)
cat("Accuracy on train set:", train_accuracy, "\n")

test_prediction <- list()
for (i in 1:length(test_indices)){
  prediction <- forward(network, iris[i, 1:4])$h[[4]]
  # print(train_prediction)
  test_predict_label <- which.max(prediction)
  test_prediction <- append(test_prediction, test_predict_label)
}
test_accuracy <- sum(test_prediction == test_labels) / length(test_prediction)
cat("Accuracy on test set:", test_accuracy, "\n")
